...
def get_max_prefill_buffer_size(vllm_config: VllmConfig):
    ...
    # 170hx-journey: 40 tokens per model token is a heuristic ceiling, not a
    # requirement. Chunked prefill bounds only the per-chunk KV span, so a
    # smaller buffer still works and leaves the memory for the KV cache.
    # GLM53_IDX_PREFILL_BUF_TOKENS caps the token count. 0 restores upstream.
    _cap = int(os.environ.get("GLM53_IDX_PREFILL_BUF_TOKENS", "4000000") or 0)
    if _cap:
