...
class MLAAttention(nn.Module, AttentionLayerBase):
    ...
    def _use_sparse_mha(self, attn_metadata: "MLACommonMetadata") -> bool:
        # 170hx-journey: backends with no dense-MHA prefill (e.g. the SM8x
        # TRITON_MLA_SPARSE/XPU-shared path, which serves prefill and decode
        # identically through the top-k sparse MQA path) never set `prefill`
        # at all, not even to None. Masked MHA is a decode-side optimization
        # this metadata type never supports either way.
