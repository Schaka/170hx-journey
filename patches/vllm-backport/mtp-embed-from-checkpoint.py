...
def _load_embed_from_checkpoint(embed: nn.Module, model_path: str) -> None:
    ...
    with safe_open(shard, framework="pt") as f:
        w = f.get_tensor(key)
    w = w.to(dtype=embed.weight.dtype, device=embed.weight.device)
    # 170hx-journey addition: the upstream PR only covers tensor-parallel-size
    # 1. Under TP>1 the draft's own embedding is a VocabParallelEmbedding
    # shard, not the full vocab, so slice the checkpoint tensor to this rank's
    # shard before copying. Without this, TP>1 crashes with a shape mismatch
    # (destination rows == vocab_size / tp_size, source rows == vocab_size).
    shard_indices = getattr(embed, "shard_indices", None)
    if shard_indices is not None and getattr(embed, "tp_size", 1) > 1:
        start = shard_indices.padded_org_vocab_start_index
        end = shard_indices.padded_org_vocab_end_index
        w = w[start:end]
    with torch.no_grad():
        # VocabParallelEmbedding pads the vocab dimension, so copy into the
        # leading rows rather than assigning the whole tensor.
        embed.weight.data[: w.shape[0]].copy_(w)
    logger.info(
