...
class DeepseekV32Model(torch.nn.Module):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        # Cross-rank indexer relay.
        # With ``index_topk_freq`` above 1 only some layers run a full
        # indexer. Every other layer reuses the selections that the last full
        # indexer wrote into ``topk_indices_buffer``. That buffer is per rank,
        # so a pipeline stage that starts on a reusing layer would attend with
        # the previous batch's selections. Carry the selections over the
        # pipeline hop and seed the receiving rank's buffer with them.
        self.index_topk = int(getattr(config, "index_topk", 0) or 0)
        self.relay_topk_indices = (
            get_pp_group().world_size > 1
            and self.index_topk > 0
            and int(getattr(config, "index_topk_freq", 1) or 1) > 1
            and os.environ.get("GLM53_PP_TOPK_RELAY", "1") == "1"
        )

        _base_empty = make_empty_intermediate_tensors_factory(
            ["hidden_states", "residual"], config.hidden_size
        )
        if self.relay_topk_indices:

            def _make_empty_with_topk(
                batch_size: int,
                dtype: torch.dtype,
                device: torch.device,
                _base: Callable = _base_empty,
                _topk: int = self.index_topk,
            ) -> IntermediateTensors:
                out = _base(batch_size, dtype, device)
                out["topk_indices"] = torch.zeros(
                    (batch_size, _topk), dtype=torch.int32, device=device
                )
                return out

            self.make_empty_intermediate_tensors = _make_empty_with_topk
        else:
            self.make_empty_intermediate_tensors = _base_empty
    ...
    def forward(
        ...
        full_num_tokens = positions.shape[0]
        if self.relay_topk_indices and not get_pp_group().is_first_rank:
            assert intermediate_tensors is not None
            self.topk_indices_buffer[:full_num_tokens, : self.index_topk] = (
                intermediate_tensors["topk_indices"][:full_num_tokens]
            )
        if self.use_sequence_parallel:
        ...
        if not get_pp_group().is_last_rank:
            ...
            if self.relay_topk_indices:
                out["topk_indices"] = self.topk_indices_buffer[
                    :full_num_tokens, : self.index_topk
                ]
            return out
