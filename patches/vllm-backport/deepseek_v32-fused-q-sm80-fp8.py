...
def _fp8_ue8m0_quantize(vals):
    ...
    ``fp8_vals`` is a raw uint8 byte, bit-exact with ``.to(tl.float8e4nv)``.

    170hx-journey addition: Triton refuses ``.to(tl.float8e4nv)`` and any
    other reference to the float8e4nv type below SM89, so this never
    constructs one. ``_encode_e4m3fn_u8`` computes the identical byte in
    software; the caller's output buffer stores it as plain uint8, and only
    reinterprets it as float8_e4m3fn on the host side, outside this kernel.
    """
    ...
    if pid == 2:
        ...
        if slot_mapping_ptr is not None:
            ...
            if MLA_CACHE_DS_MLA:
                ...
                # 170hx-journey: see _fp8_ue8m0_quantize.
...
def fused_norm_rope(
    ...
    if indexer_k_cache is not None:
        ...
        # 170hx-journey: leave as uint8 instead of viewing as float8_e4m3fn.
        # float8_e4m3fn (fp8e4nv) is unsupported below SM89, and reinterpreting
        # to it here makes Triton infer an fp8e4nv kernel parameter type at
        # compile time regardless of any runtime branch. Same byte layout,
        # same itemsize (1 byte); _fp8_ue8m0_quantize already writes the
        # bit-exact byte in software, so nothing downstream needs this view.
    else:
    ...
    if pid == 2:
        ...
        for local_head in range(2):
            ...
            if q_head_idx < NUM_Q_HEADS:
                ...
                # 170hx-journey: see _fp8_ue8m0_quantize -- Triton compiles
                # this branch's body regardless of the QUANTIZE_MQA runtime
                # guard above, so it must type-check on SM80 even though it
                # never executes there (mqa_q_fp8_ptr aliases a bf16 buffer
                # whenever QUANTIZE_MQA is False).
...
def fused_q(
    ...
    if cutedsl_kernel is not None:
        # Blackwell-only path (native fp8e4nv support): unaffected by the
        # SM80 workaround below.
    ...
    # 170hx-journey addition: allocate as uint8, not float8_e4m3fn. The
    # Triton kernel below never constructs a float8e4nv value (see
    # _fp8_ue8m0_quantize), so its output buffer must not be that dtype
    # either; reinterpret it once, on the host, right before returning.
