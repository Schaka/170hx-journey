    ...
    def update_draft_token_ids(self, draft_token_ids: DraftTokenIds) -> None:
        for req_id, spec_token_ids in zip(
            ...
            if (
                ...
                # of one request can be in flight. Drafts are only valid when
                # proposed right after that request's latest verified token. A
                # request whose newest output update_from_output just processed
                # always has num_computed_tokens == num_tokens - 1. Any other
                # state means these drafts rode on an older batch's output.
                # Keeping them lets leftover spec_token_ids make num_new_tokens
                # too large, which reschedules the request before its in-flight
                # step landed. The model runner then asserts
                # num_scheduled_tokens >= num_logits, and non-last ranks embed a
                # stale draft so their KV diverges from the last rank's.
                if request.spec_token_ids:
                    request.spec_token_ids = []
                continue
